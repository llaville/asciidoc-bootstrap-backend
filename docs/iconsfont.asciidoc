...
====
----
image:./images/icons/glyphicon/glyphicons_045_calendar.png[alt="Calendar",icon="calendar"] 10:44 05/06/2014
----
====
Renders:
...
IMPORTANT: We use here the `iconsfont` attribute, because we have already set it to another value (glyphicon) in the header block. 
...
Build this `{docfile}` footnote:[https://raw.githubusercontent.com/llaville/asciidoc-bootstrap-backend/v4/docs/iconsfont.asciidoc] with :
...
See footnote:[http://asciidoc.org/userguide.html#X88] for `iconsdir` attribute.
